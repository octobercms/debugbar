...
/**
 * OctoberBackendCollector collects backend controller and handler information
 */
class OctoberBackendCollector extends DataCollector implements Renderable
{
     ...
     */
    protected function getReflector($handler): \ReflectionClass|\ReflectionMethod
    {
        ...
        if ($handler) {
            ...
            if (strpos($handler, '::')) {
                ...
                if (
                    isset($this->controller->widget->{$widgetName}) &&
                    ($widget = $this->controller->widget->{$widgetName}) &&
                    method_exists($widget, $handlerName)
                ) {
